...
class TestFileStorage(unittest.TestCase):
    ...
    def test_save(self, mock_json_dump, mock_open_file):
        ...
        mock_open_file.assert_called_once_with(
            'file.json', 'w', encoding='utf-8')
        mock_json_dump.assert_called_once_with(
            {"BaseModel.123456": self.model_dict}, mock_open_file())

    @patch(
        'models.file_storage.open',
        new_callable=mock_open,
        read_data=(
            '{"BaseModel.123456": {"__class__": "BaseModel",'
            '"id": "123456"}}'
        )
    )
    ...
    def test_reload(self, mock_exists, mock_open_file):
        ...
        self.assertIsInstance(
            self.storage.all()["BaseModel.123456"], BaseModel)
